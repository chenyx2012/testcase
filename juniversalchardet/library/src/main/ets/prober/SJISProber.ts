...
import SJISContextAnalysis      from './contextanalysis/SJISContextAnalysis';
import SJISDistributionAnalysis from './distributionanalysis/SJISDistributionAnalysis';
import CodingStateMachine       from './statemachine/CodingStateMachine';
import SJISSMModel              from './statemachine/SJISSMModel';
...
class SJISProber extends CharsetProber {
  ...
  private contextAnalyzer: SJISContextAnalysis;
  private distributionAnalyzer: SJISDistributionAnalysis;
  ...
  public constructor() {
    super();
    this.codingSM = new CodingStateMachine(SJISProber.smModel);
  ...
  public getConfidence(): number {
    let contextCf: number = this.contextAnalyzer.getConfidence();
    let distribCf: number = this.distributionAnalyzer.getConfidence();

    if (contextCf < distribCf) {
      contextCf = distribCf;
    }
  ...
  public handleData(buf: ArrayBuffer, offset: number, length: number): ProbingState {
    ...
    for (let i: number = offset; i < maxPos; ++i) {
      codingState = this.codingSM.nextState(temp[i]);
    ...
    if (this.state == ProbingState.DETECTING) {
      if (this.contextAnalyzer.gotEnoughData() && this.getConfidence() > CharsetProber.SHORTCUT_THRESHOLD) {
        this.state = ProbingState.FOUND_IT;
      }
    }
