...
static void print_codec(const AVCodec *c)
{
    ...
    if (c->ch_layouts) {
        ...
        av_log(NULL, AV_LOG_INFO, "    Supported channel layouts:");
        while (p->nb_channels) {
            char name[128];
...
static int show_formats_devices(void *optctx, const char *opt, const char *arg, int device_only, int muxdemuxers)
{
    ...
    for (;;) {
        ...
        av_log(NULL, AV_LOG_INFO, " %c%c %-15s %s\n",
               decode ? 'D' : ' ',
               encode ? 'E' : ' ',
               name,
            long_name ? long_name:" ");
...
#   define sws_isSupportedOutput(x) 0
    ...
    while ((pix_desc = av_pix_fmt_desc_next(pix_desc))) {
        ...
        av_log(NULL, AV_LOG_INFO, "%c%c%c%c%c %-16s       %d            %3d      %d",
        ...
        for (unsigned i = 1; i < pix_desc->nb_components; i++)
            av_log(NULL, AV_LOG_INFO, "-%d", pix_desc->comp[i].depth);
        av_log(NULL, AV_LOG_INFO, "\n");
    }
    return 0;
}
...
static void print_device_list(const AVDeviceInfoList *device_list)
{
    ...
    for (int i = 0; i < device_list->nb_devices; i++) {
        ...
        av_log(NULL, AV_LOG_INFO, "%c %s [%s] (", device_list->default_device == i ? '*' : ' ',
...
static int print_device_sources(const AVInputFormat *fmt, AVDictionary *opts)
{
    ...
    if ((ret = avdevice_list_input_sources(fmt, NULL, opts, &device_list)) < 0) {
        av_log(NULL, AV_LOG_INFO, "Cannot list sources: %s\n", av_err2str(ret));
        goto fail;
    }
...
static int print_device_sinks(const AVOutputFormat *fmt, AVDictionary *opts)
{
    ...
    if ((ret = avdevice_list_output_sinks(fmt, NULL, opts, &device_list)) < 0) {
        av_log(NULL, AV_LOG_INFO, "Cannot list sinks: %s\n", av_err2str(ret));
        goto fail;
    }
...
extern void show_help_default_ffmpeg(const char *opt, const char *arg);
extern void show_help_default_ffprobe(const char *opt, const char *arg);
