...
_Thread_local char *sdp_filename;
...
_Thread_local static int file_overwrite     = 0;
_Thread_local static int no_file_overwrite  = 0;
#if FFMPEG_OPT_PSNR
_Thread_local int do_psnr            = 0;
#endif
...
static int opt_init_hw_device(void *optctx, const char *opt, const char *arg)
{
    if (!strcmp(arg, "list")) {
        enum AVHWDeviceType type = AV_HWDEVICE_TYPE_NONE;
        av_log(NULL, AV_LOG_ERROR, "Supported hardware device types:\n");
        while ((type = av_hwdevice_iterate_types(type)) !=
               AV_HWDEVICE_TYPE_NONE)
