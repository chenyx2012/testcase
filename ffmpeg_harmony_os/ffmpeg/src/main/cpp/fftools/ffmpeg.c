...
static _Thread_local int64_t nb_frames_dup = 0;
static _Thread_local uint64_t dup_warning = 1000;
static _Thread_local int64_t nb_frames_drop = 0;
static _Thread_local int64_t decode_error_stat[2];
_Thread_local unsigned nb_output_dumped = 0;
...
/* print_report */
_Thread_local static int64_t last_time = -1;
_Thread_local static int first_report = 1;
_Thread_local static int qp_histogram[52];
...
//         fflush(stderr);
//     }
    av_log(NULL, AV_LOG_INFO, "%s\n", buf.str); // 移除stats及标准输出处理;
    av_bprint_finalize(&buf, NULL);

    av_bprintf(&buf_script, "progress=%s\n", is_last_report ? "end" : "continue");
    native_report_progress(buf_script.str);

    if (progress_avio) {
        avio_write(progress_avio, (const unsigned char *)buf_script.str, FFMIN(buf_script.len, buf_script.size - 1));
...
//     received_nb_signals = 0;
    ...
    hide_banner = 0;
    for (int i = 0; i < sizeof(decode_error_stat) / sizeof(decode_error_stat[0]); i ++) {
        decode_error_stat[i] = 0;
    }
    nb_output_dumped = 0;
    nb_input_files = 0;
    nb_output_files = 0;
    ...
    program_log_flags = AV_LOG_SKIP_REPEATED;
    program_log_level = AV_LOG_INFO;
    
    last_time = -1;
    first_report = 1;
     for (int i = 0; i < sizeof(qp_histogram) / sizeof(qp_histogram[0]); i ++) {
        qp_histogram[i] = 0;
    }
...
int ffmpeg_main(_Atomic bool *is_running, int argc, char **argv)
{
    ...
    if ( exit_value != 0 ) {
        main_return_code = exit_value;
    }
    else if ( !atomic_load(is_running) ) {
        main_return_code = 255;
    }
