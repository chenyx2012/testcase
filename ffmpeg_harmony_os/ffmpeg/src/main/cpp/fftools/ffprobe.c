...
_Thread_local static int do_bitexact = 0;
_Thread_local static int do_count_frames = 0;
_Thread_local static int do_count_packets = 0;
_Thread_local static int do_read_frames  = 0;
_Thread_local static int do_read_packets = 0;
_Thread_local static int do_show_chapters = 0;
_Thread_local static int do_show_error   = 0;
_Thread_local static int do_show_format  = 0;
_Thread_local static int do_show_frames  = 0;
_Thread_local static int do_show_packets = 0;
_Thread_local static int do_show_programs = 0;
_Thread_local static int do_show_streams = 0;
_Thread_local static int do_show_stream_disposition = 0;
_Thread_local static int do_show_data    = 0;
_Thread_local static int do_show_program_version  = 0;
_Thread_local static int do_show_library_versions = 0;
_Thread_local static int do_show_pixel_formats = 0;
_Thread_local static int do_show_pixel_format_flags = 0;
_Thread_local static int do_show_pixel_format_components = 0;
_Thread_local static int do_show_log = 0;

_Thread_local static int do_show_chapter_tags = 0;
_Thread_local static int do_show_format_tags = 0;
_Thread_local static int do_show_frame_tags = 0;
_Thread_local static int do_show_program_tags = 0;
_Thread_local static int do_show_stream_tags = 0;
_Thread_local static int do_show_packet_tags = 0;

_Thread_local static int show_value_unit              = 0;
_Thread_local static int use_value_prefix             = 0;
_Thread_local static int use_byte_value_binary_prefix = 0;
_Thread_local static int use_value_sexagesimal_format = 0;
_Thread_local static int show_private_data            = 1;

#define SHOW_OPTIONAL_FIELDS_AUTO       -1
#define SHOW_OPTIONAL_FIELDS_NEVER       0
#define SHOW_OPTIONAL_FIELDS_ALWAYS      1
_Thread_local static int show_optional_fields = SHOW_OPTIONAL_FIELDS_AUTO;

_Thread_local static char *print_format;
_Thread_local static char *stream_specifier;
_Thread_local static char *show_data_hash;
...
_Thread_local static ReadInterval *read_intervals;
_Thread_local static int read_intervals_nb = 0;

_Thread_local static int find_stream_info  = 1;

static int interrupt_cb(void *ctx) {
    _Atomic bool *is_running = (_Atomic bool *)ctx;
    return !atomic_load(is_running);
//     return received_nb_signals > atomic_load(&transcode_init_done);
}

static _Thread_local AVIOInterruptCB int_cb;
...
/* FFprobe context */
_Thread_local static const char *input_filename;
_Thread_local static const char *print_input_filename;
_Thread_local static const AVInputFormat *iformat = NULL;
_Thread_local static const char *output_filename = NULL;

_Thread_local static struct AVHashContext *hash;
...
_Thread_local static int nb_streams;
_Thread_local static uint64_t *nb_streams_packets;
_Thread_local static uint64_t *nb_streams_frames;
_Thread_local static int *selected_streams;
...
_Thread_local static LogBuffer *log_buffer;
_Thread_local static int log_buffer_size;
                       ...
                       const struct section *sections, int nb_sections, const char *output)
{
    ...
    if (!output_filename) {
        ...
        if ((ret = avio_open2(&(*wctx)->avio, output, AVIO_FLAG_WRITE, &int_cb, NULL)) < 0) {
            av_log(*wctx, AV_LOG_ERROR,
...
static pthread_once_t writer_register_all_once = PTHREAD_ONCE_INIT;
static void _writer_register_all(void) {
...
static void writer_register_all(void) {
    strict_pthread_once(&writer_register_all_once, _writer_register_all);
}
...
static void print_section(SectionID id, int level)
{
    ...
    av_log(NULL, AV_LOG_ERROR, "%c%c%c",
           section->flags & SECTION_FLAG_IS_WRAPPER           ? 'W' : '.',
           section->flags & SECTION_FLAG_IS_ARRAY             ? 'A' : '.',
           section->flags & SECTION_FLAG_HAS_VARIABLE_FIELDS  ? 'V' : '.');
    av_log(NULL, AV_LOG_ERROR, "%*c  %s", level * 4, ' ', section->name);
    if (section->unique_name)
        av_log(NULL, AV_LOG_ERROR, "/%s", section->unique_name);
    av_log(NULL, AV_LOG_ERROR, "\n");
...
{
    av_log(NULL, AV_LOG_ERROR, "Sections:\n"
...
static OptionDef *opt_get_ffprobe_options(void) {
    ...
    size_t size = sizeof(real_options);
    OptionDef *ptr = (OptionDef *)malloc(size);
    memcpy(ptr, real_options, size);
    return ptr;
}
...
static void
ffprobe_reset_thread_variables(void) {
    do_bitexact = 0;
    do_count_frames = 0;
    do_count_packets = 0;
    do_read_frames  = 0;
    ...
    read_intervals = NULL; // next ...查看是否需要释放内存
    read_intervals_nb = 0;
...
int ffporbe_main(_Atomic bool *is_running, int argc, char **argv)
{
   if ( !atomic_load(is_running) ) {
    ...
    program_birth_year = ffprobe_program_birth_year; 

    fftools_init();

//     int_cb = (AVIOInterruptCB){ decode_interrupt_cb, is_running };
    ffprobe_reset_thread_variables();

    int_cb = (AVIOInterruptCB){ interrupt_cb, is_running };
...
//     init_dynload();
    ...
    if ( setjmp(exit_jump_buffer) == 0 ) {
        options = opt_get_ffprobe_options();
